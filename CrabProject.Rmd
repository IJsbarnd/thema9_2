...
were used for this project, see table 1.
...
```{r packages, echo=F, warning=FALSE}
packagelist <- c("kableExtra", "ggplot2", "factoextra", "RWeka", "reshape", "FactoMineR", "gridExtra", "forcats", "dplyr", "ggcorrplot", "gridExtra", "grid", "ggpubr")
versionlist <- c("1.3.4", "3.3.5", "1.0.7", "0.4-43", "0.8.8", "2.4", "2.3", "0.5.1", "1.0.7", "0.1.3", "2.3", "4.0.5", "0.4.0")
...
```{r Five number summary of the crab data, echo=F}
CrabSummary <- summary(myData[4:8])
CrabSummary <- sub(".*:", "", CrabSummary)
# Giving the correct row names
rownames(CrabSummary) <-  c("Minimum", "Q1", "Median", "Mean", "Q3", "Maximum")
# Giving the correct column names
colnames(CrabSummary) <- c("Frontal Lobe", "Rear Width", "Carapace Length", "Carapace Width", "Body Depth")

kable(CrabSummary, caption = "Five number summary of the morphological measurements of the crab bodies.") %>% 
kable_styling(latex_options = "hold_position")
```
...
```{r figure1, fig.cap= "Spread of Front lobe size against Carapace width based on color", echo=F, fig.height = 3.5, fig.width = 5, warning=F}
...
\newpage
```{r figure4, fig.cap="Checking if the gender makes a difference", echo=F}
...
```{r figure6, fig.cap="Checking if the gender makes a difference", echo=F}
library(ggpubr)
...
```{r ml, fig.cap="The accuracy of the machine learning algorithms ordered from low to high.", echo=F}
algorithms <- read.csv("datafiles/ml.csv")
algorithms <- data.frame(algorithms)

ggplot(algorithms, aes(x = reorder(Algorithm, desc(Accuracy)), y = Accuracy, fill = Algorithm), title="Accuracy of the machine learning algorithms") +
...
### Metalearners
```{r metalearners, echo = F, fig.cap="The accuracy of the meta machine learning algorithms ordered from low to high."}
metaalgorithms <- read.csv("datafiles/meta ml.csv")
metaalgorithms <- data.frame(metaalgorithms)

ggplot(metaalgorithms, aes(x = reorder(Algorithm, desc(Accuracy)), y = Accuracy, fill = Algorithm), title="Accuracy of the meta learners") +
  geom_bar(stat="identity", alpha=.6, width=.4) +
  coord_flip() +
  xlab("") +
  theme_bw()
...
\newpage
### ROC-curves
```{r rocs of algorithms, echo=F, fig.cap="The false positive rate against the true positive rate of the machine learning algorithms"}
ROC_build <- function(filehandle){
...
ROC_creator <- function (filefolder){
  ...
  #print(files)
  plots <- lapply(files, ROC_build)
  do.call("grid.arrange", c(plots, ncol= 3))
}
ROC_creator("rocfiles\\algorithms")
```
...
```{r, fig.cap="The metalearners: Stacking, Boosting, Bagging, Voting and Randomization", echo = FALSE}
ROC_creator("rocfiles\\meta")
```
...
## Project proposal for minor BIN
